...
Как выбрать первичный ключ из набора потенциальных?
  1. ключ должен содержать как можно меньше столбцов, чтобы быстродействие системы поиска было выше
  2. максимальное количество байт информации в столбцах выбранного ключа должно быть как можно меньше
  3. надо стремиться к числовому типу данных в ключе (быстрее обработка)
  4. столбцы ключа не должны допускать отсутствие информации в ячейках
  5. информация ключа не должна изменяться (изменение данных ключа не должно допускаться)
